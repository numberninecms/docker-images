...
test-php7:
	...
	docker container run $(DOCKER_RUN_TEST_OPTIONS) numberninecms/php:7.4-fpm-dev bash -c "php -v | grep '7\.4' && php -v | grep 'Xdebug'"

test-php8:
	docker container run $(DOCKER_RUN_TEST_OPTIONS) numberninecms/php:8.0-fpm bash -c "php -v | grep '8\.0'"
	docker container run $(DOCKER_RUN_TEST_OPTIONS) numberninecms/php:8.0-fpm-dev bash -c "php -v | grep '8\.0' && php -v | grep 'Xdebug'"
